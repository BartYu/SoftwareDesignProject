...
const handleAddEvent = () => {
    if (selectedEventId) {
        ...
        if (selectedEvent) {
        const formattedDate = new Date(currentDate.getFullYear(), currentDate.getMonth(), selectedDate);

        const eventExists = events.some(event => event.title === selectedEvent.title);
        if (eventExists) {
            alert("This event already exists and cannot be added again.");
            return;
        }
...
return (
    ...
    <div className="modal">
        <div>
            ...
            <select onChange={(e) => setSelectedEventId(Number(e.target.value))} value={selectedEventId || ''}>
...
{events.length === 0 ? (
    ...
    events.map((event) => {
    const date = event.date.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
    return (
